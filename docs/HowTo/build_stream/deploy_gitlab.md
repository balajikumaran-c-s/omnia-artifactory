...
- Ensure the OIM and GitLab hosts have working DNF repositories. The deployment installs `sshpass` on the OIM and installs `curl`, `policycoreutils`, `policycoreutils-python-utils`, `openssl`, `firewalld`, `podman`, and `podman-docker` on the GitLab host.
- Provide root SSH access from the OIM to the GitLab host. The deployment initially uses the supplied root SSH password and then authorizes `/root/.ssh/omnia_gitlab.pub` for passwordless access.
- Disable SELinux on the GitLab host and reboot it before deployment. The prerequisite check fails while SELinux is enabled.
