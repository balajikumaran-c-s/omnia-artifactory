...
| `additional_cloud_init.yml` | Currently unavailable | The input contract is retained, but the current runtime does not propagate its configured path to the OpenCHAMI role; the file is therefore not applied. |
...
| `credentials` | Create or load the Vault-encrypted Orchestrator credentials and prompt for required values that are still empty. Existing non-empty values are retained. | Current catalog and the applicable credential values. Upstream status files are not required. |
...
service readiness, provisioning, and conditional PXE boot. Cleanup, upgrade,
and rollback are tagged `never` and do not run in this mode.
