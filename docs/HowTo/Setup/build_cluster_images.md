...
Before building aarch64 images, you must install RHEL 10 on one of the aarch64 nodes. You can use the [Install OS via iDRAC Virtual Media](../utils/install_os_unattended.md) workflow to automate this, or install manually:

1. Ensure a disk is available to the aarch64 node for OS installation.

2. Install the **full Red Hat Enterprise Linux 10 OS** on one of the aarch64 nodes with the root password enabled. Use the [Install OS via iDRAC Virtual Media](../utils/install_os_unattended.md) procedure for automated installation, or install manually.
